...
import com.lunchandmeet.utils.ApiKeys;
...
public class OdnoklassnikiOAuth20ServiceImpl extends OAuth20ServiceImpl {
    ...
    public void signRequest(Token accessToken, OAuthRequest request) {
        publicKey = ApiKeys.ODNOKLASSNIKI_API_PUBLIC;
        if (null == publicKey || publicKey.isEmpty()) {
        ...
        String sign = UserDaoMongo.md5Java("application_key=" + publicKey + "format=jsonmethod=users.getCurrentUser"
                +
                UserDaoMongo.md5Java(accessToken.getToken() + config.getApiSecret()))
                ;
        request.addQuerystringParameter("application_key", this.publicKey);
        request.addQuerystringParameter("method", "users.getCurrentUser");
        request.addQuerystringParameter("sig", sign);
        request.addQuerystringParameter("format", "json");
        request.addQuerystringParameter("access_token", accessToken.getToken().trim());
    }

    private String calculateSig2(String parametersMap, String accessTokenValue, String apiSecret) {
        String accessTokenPlusSecretMd5 = HexStringsConverter.toHexString(messageDigest.digest((accessTokenValue + apiSecret).getBytes()));
        String wholeStringMd5 = HexStringsConverter.toHexString(messageDigest.digest((parametersMap + accessTokenPlusSecretMd5).getBytes()));
        return wholeStringMd5;
    }
}
